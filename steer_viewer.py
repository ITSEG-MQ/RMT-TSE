...
def draw_pt(img, x, y, color, sz=1, offset=0):
...
#     # for i in range(int(row - sz), int(row + sz)):
#     #     for j in range(col - sz, col + sz):
#     if row >= 0 and row < img.shape[0] and col >= 0 and col < img.shape[1]:
#         img[int(row)-sz:int(row)+sz, int(col)-sz:int(col)+sz] = img[int(row)-sz:int(row)+sz, int(col)-sz:int(col)+sz] * (1 - trans) + np.array(color) * trans
#     # if not (i < 0 or i >= width) and not (j < 0 or j >= height):
#     #     img[i][j] = img[i][j] * (1 - trans) + color * trans

def draw_path(img, path_x, path_y, color, offset=0):
  for x, y in zip(path_x, path_y):
    draw_pt(img, x, y, color, offset=offset)
...
def draw_path_on(img, speed_ms, angle_steers, color=(0, 255, 0), offset=0):
  path_x = np.arange(0., 50.1, 0.5)
  path_y, _ = calc_lookahead_offset(speed_ms, angle_steers, path_x)
  draw_path(img, path_x, path_y, color, offset)
...
def draw(img, y_pred, y_true=None):
    ...
    if y_true:
        draw_path_on(img, speed_ms, y_true/5.0, red)
        draw_path_on(img, speed_ms, y_pred/5.0, offset=10)
    else:
...
if __name__ == "__main__":
  img = 'test_images/x_n1/1533646242154970.png'
  draw_img = draw(cv2.imread(img), -0.5)
  cv2.imwrite('image_with_steering.png', draw_img)
