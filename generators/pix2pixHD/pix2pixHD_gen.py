...
def image_control_complex(dataset_path, output_path):
    file_name = 0
    for i, data in enumerate(dataset):
        if i < len(dataset):
            ...
            try:
                ...
                feat_map, new_data = replace_object(feat_map, new_data, semantic_class=[35], replace_class=26)
                generated3, feat_map = model.inference_gen(new_data['label'], new_data['inst'], new_data['image'],
                                                            feat_map=feat_map)
                ...
                save_image(util.tensor2im(generated3.data[0]),
                            output_path + '/x_n3', img_name_by_json)
...
def replace_object(feat_map, data, semantic_class=[11, 32], replace_class=33):
    ...
    for c in semantic_class:
        replace_region.append((data['label'] == c).nonzero().cpu())
    ...
    data['inst'] = torch.from_numpy(data['inst'])
